...
public sealed class SingleMbTilesSource : IVectorTileSource, IDisposable
{
    ...
    public void ExtractTile(int x, int y, int z, string path)
    {
        if (File.Exists(path))
        {
            File.Delete(path);
        ...
        using Stream tileStream = GetRawTile(x, y, z);
    ...
    public Task<VectorTile> GetVectorTile(int x, int y, int z)
    {
        ...
        if (z > MaxZoom)
        {
            var bounds = gmt.TileLatLonBounds(x, y, z);
        ...
        try
        {
            var actualTile = GetCachedVectorTile(x, y, z);
    ...
    private VectorTile GetCachedVectorTile(int x, int y, int z)
    {
        var key = (x, y, z);
        ...
        lock (keyLock)
        {
            ...
            using var rawTileStream = GetRawTile(x, y, z);
            ...
            var tile = pbfTileProvider.GetVectorTile(x, y, z).GetAwaiter().GetResult();
            tileCache[key] = tile;
